...
for index, row in tqdm(df_directorys.iterrows(), total=df_directorys.shape[0]):
    # 构建文件路径
    file_path = os.path.join(*[comp for comp in [row['Level_0'], row['Level_1'], row['Level_2'], row['Level_3'], row['Level_4'], row['Level_5'], row['Level_6']] if comp is not None])
...
# %%
expanded_df.to_excel('周期计算.xlsx', index=None)

# %% 计算周期均值并生成表格（添加到现有代码末尾）

# 过滤掉正常数据，只保留故障类型为B、IR、OR的数据
fault_data = expanded_df[expanded_df['故障类型'].isin(['B', 'IR', 'OR'])]

# 计算BPFO周期均值表
bpfo_means = fault_data.groupby(['Level_2', '故障类型'])['N_BPFO'].mean().unstack(fill_value=0)
bpfo_means = bpfo_means.reindex(columns=['B', 'IR', 'OR'], fill_value=0)

# 计算BPFI周期均值表
bpfi_means = fault_data.groupby(['Level_2', '故障类型'])['N_BPFI'].mean().unstack(fill_value=0)
bpfi_means = bpfi_means.reindex(columns=['B', 'IR', 'OR'], fill_value=0)

# 计算BSF周期均值表
bsf_means = fault_data.groupby(['Level_2', '故障类型'])['N_BSF'].mean().unstack(fill_value=0)
bsf_means = bsf_means.reindex(columns=['B', 'IR', 'OR'], fill_value=0)

# 打印表格
print("表：不同采集频率以及故障类型的 BPFO 周期均值")
print("| 数据源 | B | IR | OR |")
print("|---|---|---|---|")
for source in ['12kHz_DE_data', '12kHz_FE_data', '48kHz_DE_data']:
    if source in bpfo_means.index:
        b_val = f"{bpfo_means.loc[source, 'B']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfo_means.loc[source, 'B']) else 'N/A'
        ir_val = f"{bpfo_means.loc[source, 'IR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfo_means.loc[source, 'IR']) else 'N/A'
        or_val = f"{bpfo_means.loc[source, 'OR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfo_means.loc[source, 'OR']) else 'N/A'
        print(f"| {source} | {b_val} | {ir_val} | {or_val} |")

print("\n表：不同采集频率以及故障类型的 BPFI 周期均值")
print("| 数据源 | B | IR | OR |")
print("|---|---|---|---|")
for source in ['12kHz_DE_data', '12kHz_FE_data', '48kHz_DE_data']:
    if source in bpfi_means.index:
        b_val = f"{bpfi_means.loc[source, 'B']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfi_means.loc[source, 'B']) else 'N/A'
        ir_val = f"{bpfi_means.loc[source, 'IR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfi_means.loc[source, 'IR']) else 'N/A'
        or_val = f"{bpfi_means.loc[source, 'OR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bpfi_means.loc[source, 'OR']) else 'N/A'
        print(f"| {source} | {b_val} | {ir_val} | {or_val} |")

print("\n表：不同采集频率以及故障类型的 BSF 周期均值")
print("| 数据源 | B | IR | OR |")
print("|---|---|---|---|")
for source in ['12kHz_DE_data', '12kHz_FE_data', '48kHz_DE_data']:
    if source in bsf_means.index:
        b_val = f"{bsf_means.loc[source, 'B']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bsf_means.loc[source, 'B']) else 'N/A'
        ir_val = f"{bsf_means.loc[source, 'IR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bsf_means.loc[source, 'IR']) else 'N/A'
        or_val = f"{bsf_means.loc[source, 'OR']:.10f}".rstrip('0').rstrip('.') if pd.notna(
            bsf_means.loc[source, 'OR']) else 'N/A'
        print(f"| {source} | {b_val} | {ir_val} | {or_val} |")

# 将结果保存到Excel文件
with pd.ExcelWriter('周期计算结果.xlsx') as writer:
    expanded_df.to_excel(writer, sheet_name='详细数据', index=False)

    # 保存均值表格
    bpfo_means.to_excel(writer, sheet_name='BPFO周期均值')
    bpfi_means.to_excel(writer, sheet_name='BPFI周期均值')
    bsf_means.to_excel(writer, sheet_name='BSF周期均值')
