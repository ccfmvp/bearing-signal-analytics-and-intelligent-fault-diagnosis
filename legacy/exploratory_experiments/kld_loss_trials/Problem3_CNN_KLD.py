...
def train_model_with_mmd(model, train_loader, target_data, criterion, optimizer, epochs=100, mmd_weight=0.25):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            mmd = kld_loss(source_features, target_features)
        ...
        print(f'Epoch [{epoch + 1}/{epochs}], Total Loss: {epoch_loss:.4f}, '
              f'Class Loss: {epoch_class_loss:.4f}, KLD Loss: {epoch_mmd_loss:.4f}')
...
for train_index, val_index in kf.split(X_source_scaled, y_encoded):
    ...
    report_df.to_csv(f'KLD/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'KLD/fold_{fold}_confusion_matrix.csv')
    ...
    plt.tight_layout()
    plt.savefig(f'KLD/fold_{fold}_confusion_matrix.png', dpi=150)
...
# 保存带有预测结果的目标域数据
target_data_with_predictions.to_csv('KLD/target_data_with_predictions.csv', index=False)
print("目标域数据预测完成并已保存到 'KLD/target_data_with_predictions.csv'")

result_data = pd.read_csv('KLD/target_data_with_predictions.csv', index_col=None)
...
# 显示结果
result_df.to_csv('KLD 问题三 7 分类 Fold 1-5 结果.csv', index=None)
...
final_df.to_csv('KLD 问题三 7 分类 A-P 结果.csv', index=None)