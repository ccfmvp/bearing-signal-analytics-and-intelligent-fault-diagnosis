...
# 创建保存结果的目录
os.makedirs('result/XGBoost23', exist_ok=True)
...
for train_index, val_index in kf.split(X, y_encoded):
    ...
    report_df.to_csv(f'result/XGBoost23/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'result/XGBoost23/fold_{fold}_confusion_matrix.csv')
    ...
    plt.tight_layout()
    plt.savefig(f'result/XGBoost23/fold_{fold}_confusion_matrix.png', dpi=150)
    plt.show()
...
plt.tight_layout()
plt.savefig('result/XGBoost23/all_folds_confusion_matrix_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
overall_stats.to_csv('result/XGBoost23/overall_statistics.csv')