...
class CNNBiLSTM(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.cnn_features = nn.Sequential(
            ...
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),  # 只保留一次池化
        ...
        self.lstm = nn.LSTM(
            input_size=256,
            hidden_size=64,
            num_layers=2,  # 增加到2层以使用dropout
            batch_first=True,
            bidirectional=True,
            dropout=0.2  # 现在可以保留dropout
        )
...
for train_index, val_index in kf.split(X_scaled, y_encoded):
    ...
    # 初始化模型、损失函数和优化器
    model = CNNBiLSTM(input_dim=X.shape[1], num_classes=len(le.classes_)).to(device)
    ...
    report_df.to_csv(f'result/CNNBiLSTM23/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'result/CNNBiLSTM23/fold_{fold}_confusion_matrix.csv')
    ...
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM23/fold_{fold}_metrics.png', dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM23/fold_{fold}_confusion_matrix.png', dpi=150)
    plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNBiLSTM23/all_folds_metrics_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNBiLSTM23/all_folds_confusion_matrix_summary.png', dpi=150, bbox_inches='tight')
plt.show()

# 保存模型示例（保存最后一个fold的模型）
torch.save(model.state_dict(), 'model/cnn_bilstm_model.pth')
print("模型已保存为 cnn_bilstm_model.pth")
