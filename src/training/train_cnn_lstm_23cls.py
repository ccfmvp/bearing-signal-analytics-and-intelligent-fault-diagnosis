...
class CNNLSTM(nn.Module):
    def __init__(self, input_dim, num_classes, lstm_hidden_size=128, num_layers=2):
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(lstm_hidden_size * 2, 128),  # 双向LSTM所以×2
            nn.ReLU(),
            nn.BatchNorm1d(128),
            nn.Dropout(0.3),
    ...
    def forward(self, x):
        ...
        x = self.cnn_features(x)  # [batch, 256, seq_len//4]
...
for train_index, val_index in kf.split(X_scaled, y_encoded):
    ...
    # 初始化模型、损失函数和优化器
    model = CNNLSTM(input_dim=X.shape[1], num_classes=len(le.classes_)).to(device)
    ...
    report_df.to_csv(f'result/CNNLSTM23/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'result/CNNLSTM23/fold_{fold}_confusion_matrix.csv')
    ...
    plt.tight_layout()
    plt.savefig(f'result/CNNLSTM23/fold_{fold}_metrics.png', dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'result/CNNLSTM23/fold_{fold}_confusion_matrix.png', dpi=150)
    plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNLSTM23/all_folds_metrics_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNLSTM23/all_folds_confusion_matrix_summary.png', dpi=150, bbox_inches='tight')
plt.show()

# 保存模型示例（保存最后一个fold的模型）
torch.save(model.state_dict(), 'model/cnn_lstm_model.pth')
print("模型已保存为 cnn_lstm_model.pth")
