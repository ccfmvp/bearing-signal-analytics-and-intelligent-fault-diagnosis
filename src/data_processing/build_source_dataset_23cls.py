...
# %%
def extract_size_from_row(row):
    """
    从Level_4或Level_5中提取尺寸信息
    如果找到纯数字，则返回该数字；否则返回None
    """
    # 检查Level_4
    if pd.notna(row['Level_4']):
        level_4_str = str(row['Level_4'])
        # 使用正则表达式匹配纯数字
        match = re.search(r'^\d+$', level_4_str)
        if match:
            return match.group()

    # 检查Level_5
    if pd.notna(row['Level_5']):
        level_5_str = str(row['Level_5'])
        # 使用正则表达式匹配纯数字
        match = re.search(r'^\d+$', level_5_str)
        if match:
            return match.group()
...
for index, row in tqdm(expanded_df.iterrows(), total=expanded_df.shape[0]):
    ...
    all_features['尺寸'] = size if size else 'N'
...
def create_label(row):
    base_label = f"{row['故障类型']}--{row['轴承类别']}--{row['尺寸']}"
    if row['故障类型'] == 'N':
        return "N--N--N"
    return base_label
...
# %%
# 保存结果
final_dataframe.to_csv('labeledData23.csv', index=None)
print(f"\n数据已保存到 labeledData23.csv")