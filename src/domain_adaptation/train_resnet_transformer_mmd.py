...
# 创建保存结果的目录
os.makedirs('result/ResNetTransformer_MMD', exist_ok=True)
...
class ResNetTransformer(nn.Module):
    def __init__(self, input_dim, num_classes, d_model=128, nhead=8, num_layers=2, dim_feedforward=512, dropout=0.2):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=False  # 注意：PyTorch 默认是 [seq, batch, feat]
        )
...
class CombinedModel(nn.Module):
    def __init__(self, linear_input_dim, linear_output_dim, num_classes):
        super(CombinedModel, self).__init__()
        self.linear_mapping = LinearMapping(linear_input_dim, linear_output_dim)
        self.resnet_transformer = ResNetTransformer(
            input_dim=linear_output_dim,
...
for train_index, val_index in kf.split(X_source_scaled, y_encoded):
    ...
    report_df.to_csv(f'result/ResNetTransformer_MMD/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'result/ResNetTransformer_MMD/fold_{fold}_confusion_matrix.csv')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/ResNetTransformer_MMD/fold_{fold}_loss_curves.png', dpi=150)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/ResNetTransformer_MMD/fold_{fold}_component_losses.png', dpi=150)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/ResNetTransformer_MMD/fold_{fold}_validation_metrics.png', dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'result/ResNetTransformer_MMD/fold_{fold}_confusion_matrix.png', dpi=150)
    plt.show()
...
plt.tight_layout()
plt.savefig('result/ResNetTransformer_MMD/all_folds_metrics_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('result/ResNetTransformer_MMD/all_folds_confusion_matrix_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
# 保存带有预测结果的目标域数据
target_data_with_predictions.to_csv('result/ResNetTransformer_MMD/target_data_with_predictions.csv', index=False)

# 读取结果数据
result_data = pd.read_csv('result/ResNetTransformer_MMD/target_data_with_predictions.csv', index_col=None)
...
# 保存详细结果
result_df.to_csv('result/ResNetTransformer_MMD/问题三_4分类_Fold_1-5_结果.csv', index=None)
...
final_df.to_csv('result/ResNetTransformer_MMD/问题三_4分类_A-P_结果.csv', index=None)
