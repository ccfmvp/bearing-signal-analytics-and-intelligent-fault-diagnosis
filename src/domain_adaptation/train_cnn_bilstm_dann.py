...
# 创建保存结果的目录
os.makedirs('result/CNNBiLSTM_DANN', exist_ok=True)
...
class CNNBiLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, cnn_out_channels=64):
        ...
        self.feature_dim = hidden_dim * 2  # 双向 LSTM
    ...
    def forward(self, x):
        ...
        x, _ = self.lstm(x)
        x = x[:, -1, :]  # (batch, hidden_dim * 2) —— 提取最后一个时间步作为特征
        return x  # 返回特征
...
class CombinedModel(nn.Module):
    def __init__(self, linear_input_dim, linear_output_dim, cnnbilstm_input_dim, num_classes):
        ...
        self.cnn_bilstm = CNNBiLSTM(input_dim=cnnbilstm_input_dim)  # 只提取特征
        self.classifier = nn.Linear(self.cnn_bilstm.feature_dim, num_classes)  # 分类头
        self.domain_classifier = nn.Sequential(
            nn.Linear(self.cnn_bilstm.feature_dim, 128),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(128, 2)
        )
        self.grl = GradientReversalLayer()  # 使用前面定义的新式 GRL

    def forward(self, x, alpha=1.0):
        x = self.linear_mapping(x)               # (B, seq_len, D)
        features = self.cnn_bilstm(x)            # (B, feature_dim)
        class_logits = self.classifier(features) # (B, num_classes)
        domain_logits = self.domain_classifier(self.grl(features, alpha))  # (B, 2)
...
# ==================== 训练和评估函数 ====================
def train_model_with_dann(model, train_loader, val_loader, target_data, criterion, domain_criterion, optimizer,
                          epochs=100, alpha=1.0, domain_weight=0.5):
    model.train()
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            # 获取源域分类输出和域输出
            outputs, domain_outputs = model(inputs, alpha)

            # 获取目标域域输出
            _, target_domain_outputs = model(target_subset, alpha)

            # 计算域标签（源域为0，目标域为1）
            domain_labels = torch.zeros(len(inputs), dtype=torch.long, device=device)
            domain_labels_target = torch.ones(len(target_subset), dtype=torch.long, device=device)

            # 计算分类损失
            class_loss = criterion(outputs, labels)

            # 计算域分类损失
            domain_loss = domain_criterion(domain_outputs, domain_labels) + domain_criterion(target_domain_outputs,
        ...
        epoch_domain_loss = running_domain_loss / len(train_loader.dataset)

        train_losses.append(epoch_loss)
        class_losses.append(epoch_class_loss)
        domain_losses.append(epoch_domain_loss)
        ...
        if (epoch + 1) % 10 == 0:
            print(f'Epoch [{epoch + 1}/{epochs}], Total Loss: {epoch_loss:.4f}, '
                  f'Class Loss: {epoch_class_loss:.4f}, Domain Loss: {epoch_domain_loss:.4f}')
...
for train_index, val_index in kf.split(X_source_scaled, y_encoded):
    ...
    domain_criterion = nn.CrossEntropyLoss()  # 域分类损失函数
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-5)

    # 训练模型（带DANN）
    print("开始训练模型...")
    history = train_model_with_dann(model, train_loader, val_loader, X_target_tensor,
                                    criterion, domain_criterion, optimizer, epochs=50, alpha=0.5, domain_weight=0.5)
    ...
    report_df.to_csv(f'result/CNNBiLSTM_DANN/fold_{fold}_classification_report.csv')
    ...
    cm_df.to_csv(f'result/CNNBiLSTM_DANN/fold_{fold}_confusion_matrix.csv')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM_DANN/fold_{fold}_loss_curves.png', dpi=150)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM_DANN/fold_{fold}_component_losses.png', dpi=150)
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM_DANN/fold_{fold}_validation_metrics.png', dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'result/CNNBiLSTM_DANN/fold_{fold}_confusion_matrix.png', dpi=150)
    plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNBiLSTM_DANN/all_folds_metrics_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('result/CNNBiLSTM_DANN/all_folds_confusion_matrix_summary.png', dpi=150, bbox_inches='tight')
plt.show()
...
# 保存带有预测结果的目标域数据
target_data_with_predictions.to_csv('result/CNNBiLSTM_DANN/target_data_with_predictions.csv', index=False)

# 读取结果数据
result_data = pd.read_csv('result/CNNBiLSTM_DANN/target_data_with_predictions.csv', index_col=None)
...
# 保存详细结果
result_df.to_csv('result/CNNBiLSTM_DANN/问题三_4分类_Fold_1-5_结果.csv', index=None)
...
final_df.to_csv('result/CNNBiLSTM_DANN/问题三_4分类_A-P_结果.csv', index=None)
