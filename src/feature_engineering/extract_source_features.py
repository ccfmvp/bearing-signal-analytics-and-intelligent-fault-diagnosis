...
# 读取文件并提取特征，保存为CSV文件
expanded_df = pd.read_excel('cycle.xlsx', index_col=None)  # 这个文件是包含数据源的Excel

# for index, row in tqdm(expanded_df.iterrows(), total=expanded_df.shape[0]):
#     all_features = []
#     df = pd.read_csv(f'original_sliding_window_result/{index + 1:03d}.csv', index_col=None, header=0)
...
#
#     # 保存特征到CSV文件
#     all_features.to_csv(f'{csv_folder}/特征提取_{index + 1:03d}.csv', index=None)


start_index = 350  # 设置起始索引

# 只处理从start_index开始的行
for index, row in tqdm(expanded_df.iloc[start_index:].iterrows(), total=expanded_df.shape[0]-start_index):
    all_features = []
    df = pd.read_csv(f'original_sliding_window_result/{index + 1:03d}.csv', index_col=None, header=0)
    fs = 48000 if row['Level_2'].startswith('48') else 12000
    file_features = []
